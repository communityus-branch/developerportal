---
title:  Open source libraries used by Affdex
permalink: /licenses/
metadata: false
---
Affdex SDKs uses the following open source libraries. Find below the links to the licenses for each:

### Android

* [Dagger](http://square.github.io/dagger/#license)
* [javax.inject](http://www.apache.org/licenses/LICENSE-2.0.txt)

### Windows & Linux

* [SHA-256 implementation](http://www.zedwood.com/article/cpp-sha256-function)
* [FFmpeg](http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html)

### Multiple Platforms

* [EmojiOne](http://emojione.com/licensing/)
* [Boost](http://www.boost.org/LICENSE_1_0.txt)
* [OpenCV](http://opencv.org/license.html)
* [Haar cascade classifier files for OpenCV](https://github.com/Itseez/opencv/blob/master/data/haarcascades/haarcascade_frontalface_alt2.xml)
